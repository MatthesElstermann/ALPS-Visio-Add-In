...
namespace VisioAddIn.OwlShapes.util
{
    public class SBDPageExportHelper : PageExportHelper
    {
        ...
        {
            vis.Document shapes = VisioHelper.openStencil(VisioHelper.VisioStencils.SBD_STENCIL); ;
            ...
            if (shapes != null)
            {
                vis.Master sbdMaster = shapes.Masters.get_ItemU(masterType);
                ...
                // Drop the shape
                vis.Shape droppedShape = page.Drop(sbdMaster, simpleXPos, simpleYPos);
                return droppedShape;
            }
